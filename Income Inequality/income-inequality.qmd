...
g1995a <- g1995 +
  annotate(
    ...
    label = "Test", colour = text_col, family = body_font
  ) +
  annotate(
...
Let's start by looking at just one of the curved lines - single year.

The vertical (y) axis shows the percentage of each countries income (% income) that was received by the richest 1% of the country's population in that year.

Higher values mean a more unequal distribution of income.
...
These lines do not change when selecting a different country.
...
g1995b <- g1995 +
  annotate(
    geom = "text", x = 10, y = 1.2,
    label = "density", colour = text_col, family = body_font
  ) +
  annotate(
    geom = "text", x = 23, y = 1.2,
    label = "Test", colour = text_col, family = body_font
...
In this chart, the horizontal (x) axis shows different years from 1980 to 2023. Not all countries have data for all years.

The points that are highlighted when hovering or selecting via the dropdown show the values for specific country in each year where data is available. Going from left to right when looking at the points allows you to see how it is changing over time. Points going upwards, means that country is becoming more unequal.
